...
public class RefreshingOAuth2ConnectionFactory<S> extends OAuth2ConnectionFactory<S>
{
    ...
    public RefreshingOAuth2ConnectionFactory(String providerId,
                                             OAuth2ServiceProvider<S> soAuth2ServiceProvider,
                                             ApiAdapter<S> sApiAdapter)
    {
    ...
    {
        return new RefreshingOAuth2Connection<S>(getProviderId(),
                                                 extractProviderUserId(accessGrant),
                                                 accessGrant.getAccessToken(),
                                                 accessGrant.getRefreshToken(),
                                                 accessGrant.getExpireTime(),
                                                 (OAuth2ServiceProvider<S>) getServiceProvider(),
                                                 getApiAdapter());
    }
    ...
    {
        return new RefreshingOAuth2Connection<S>(data,
                                                 (OAuth2ServiceProvider<S>) getServiceProvider(),
                                                 getApiAdapter());
    }
