...
public class ResultItemDeserializer extends JsonDeserializer<ResultItem>
{
    ...
    @Override
    public ResultItem deserialize(JsonParser jp,
        ...
        for (Map.Entry<String, Object> entry : map.entrySet()) {
            if (entry.getValue() instanceof Map) {
                ...
                if (inner.get("records") != null) {
                    item.getAttributes().put(entry.getKey(), mapper.readValue(jp, QueryResult.class));
                } else {
                    item.getAttributes().put(entry.getKey(), mapper.readValue(jp, ResultItem.class));
                }
            } else {
